...
/**
 * An image filters which implements the ordered dithering.
 * <p>
 * It's often necessary to reduce the number of colours used in an image,
 * usually in order to display it on a device which has colour limitations. For
 * example, we may want to display a colour image on a black and white printer.
 ...
 * </p>
 * <p>
 * The easiest method of halftoning is ordered dithering. In this method, pixels
 * are turned on or off according to a pattern called a dither matrix. The dither
 * matrix is a rectangular array of numbers, all different, which define the
 * order in which pixels are turned on as the brightness of a pixel increases.
 * The matrix is tiled over the image and the pixel at each point is turned on
 * if its brightness is larger than the value in the matrix at that point,
 * suitably scaled for the number of levels we're dithering to.
 * </p>
 * <p>Here's the simplest 2x2 dither matrix:</p>
 * <p><code>
 *    0 2<br>
 *    3 1<br>
 * </code></p>
 * <p>And here's a 4x4 dither matrix:</p>
 * <p><code>
 *   0 14  3 13<br>
 *  11  5  8  6<br>
 *  12  2 15  1<br>
 ...
 * </code></p>
 * <p>
 * This filter dithers an image to a specified number of colours. You supply
 * the number of colors and the dither matrix. There are built-in matrices for
 * common patterns such as 4x4 magic square and ordered dither etc.
 * </p>
 *
 *
